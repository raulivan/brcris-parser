...
def formart_date_str_eng_to_ptbr(data:str) -> tuple[str,int]:
    """
...
def monta_areas_do_conhecimento(grande_area:str, area_do_conhecimento:str, sub_area:str, especialidade:str) -> str:
    ...
    area_formartado = nbr_title(area_do_conhecimento.replace("_", " "))
    retorno = f"{retorno} / {area_formartado}"
    
    if sub_area is None or sub_area.strip() == '':
        return retorno
    
    sub_area_formatado = nbr_title(sub_area.replace("_", " "))
    retorno = f"{retorno} / {sub_area_formatado}"

    if especialidade is None or especialidade.strip() == '':
        return retorno
    
    especialidade_formatado = nbr_title(especialidade.replace("_", " "))
    retorno = f"{retorno} / {especialidade_formatado}"
